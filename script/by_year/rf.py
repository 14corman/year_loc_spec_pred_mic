...
import os
from data_manipulation import get_data, translate_pm1, save_results_rf, get_year_data, get_labels, save_genotypes, save_pred_mic_distro
import csv
import math
...
def truncate_mics(mics):
    trunc_mics = []
    for mic in mics:
        if TRUNC_SIR:
            if mic <= 2.0:
                trunc_mics.append(0) # S
            elif mic > 8.0:
                trunc_mics.append(2) # R
            else:
                trunc_mics.append(1) #I
        else:
            if mic <= 0.12:
                trunc_mics.append(0.12)
            elif mic >= 32.0:
                trunc_mics.append(32.0)
            else:
                trunc_mics.append(mic)
...
def main():
    ...
    for train_antibiotic in [3, 6, 13, 21, 32, 34, 41]:
        ...
        if TRUNCATE:
            y_train = truncate_mics(y_train)
        
        labels = get_labels(os.getcwd(), 'labels.csv', train_antibiotic)
        
        if TRUNCATE:
            print(labels)
            labels = list(set(truncate_mics(labels)))
            print(labels)
        ...
        if col_header == EXTRACT_DRUG:
            save_genotypes('saved_genotypes', correct, incorrect, 'rf')
            save_genotypes('saved_genotypes_mics', correct_mics, incorrect_mics, 'rf')
            ...
            if not TRUNCATE and not TRUNC_SIR:
                ...
                save_pred_mic_distro("prediction_distro", correction_output, 'rf')
