...
def load_dataset():
        ...
        for row in csvReader:
            if lineCount == 0:
                ...
                if row[species_cell] in passing_organisms:
                    ...
                    row_data = dict()
                    row_data["id"] = row[collection_num_cell]
                    row_data["s"] = row[species_cell]
                    row_data["c"] = row[country_cell]
                    row_data["y"] = row[year_cell]
                    row_data["m"] = mics
                    row_data["g"] = genes
    ...
    for r in all_rows:
        ...
        if year == '2016' or year == '2017':
            #Have each row contain all MIC data and gene data for an isolate
            #if that isolate was collected in 2016 or 2017
            data_point = [r.get('id')]
            for a in m:
                data_point.append(a)
            ...
            if year in years:
                years[year].append(data_point)
            else:
                years[year] = [data_point]
        else:
